...
func SelfCheck(ctx context.Context, addr string, handler http.Handler) error {
	...
	pageResponse, err := client.Get(base + "/workbench")
	if err != nil {
	...
	if err := call(client, "POST", base+"/api/v1/dossiers", map[string]any{"trenchCode": "SC-T01", "excavationArea": "自检区", "leadRecorder": "记录员甲", "chronologyHypothesis": "汉代", "actor": "记录员甲", "idempotencyKey": "self-create-001"}, &snapshot); err != nil {
		return err
	}
	...
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/units/batch", batch, &batchResult); err != nil {
		return err
	}
	...
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/relations", relation, &snapshot); err != nil {
		return err
	}
	var path domain.RelationPathResult
	if err := call(client, "GET", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/relation-path?sourceUnitId="+firstID+"&targetUnitId="+secondID, nil, &path); err != nil {
		return err
	}
	if path.Classification != "direct" || len(path.Path) != 1 {
		return fmt.Errorf("关系路径追溯断言失败")
	}
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/checks", versioned(snapshot, "记录员甲", "self-check-001"), &snapshot); err != nil {
		return err
	}
	...
	if err := call(client, "GET", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/checks", nil, &batches); err != nil {
		return err
	}
	if len(batches) != 1 || batches[0].ID != snapshot.LastCheckRunID {
		return fmt.Errorf("检查批次查询断言失败")
	}
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/submit", versioned(snapshot, "记录员甲", "self-submit-001"), &snapshot); err != nil {
		return err
	}
	...
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/review", review, &snapshot); err != nil {
		return err
	}
	...
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/credentials", issue, &snapshot); err != nil {
		return err
	}
	...
	if err := call(client, "GET", base+"/api/v1/credentials/"+snapshot.Credentials[0].CredentialID, nil, &verification); err != nil {
		return err
	}
	...
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/credentials/"+oldID+"/revoke", revoke, &snapshot); err != nil {
		return err
	}
	if err := call(client, "GET", base+"/api/v1/credentials/"+oldID, nil, &verification); err != nil {
		return err
	}
	if verification.Valid {
		return fmt.Errorf("已撤销凭据不应有效")
	}
	if err := call(client, "POST", base+"/api/v1/dossiers/"+snapshot.Dossier.ID+"/credentials/"+oldID+"/reissue", versioned(snapshot, "负责人丙", "self-reissue-001"), &snapshot); err != nil {
		return err
	}
	newID := snapshot.Credentials[1].CredentialID
	if err := call(client, "GET", base+"/api/v1/credentials/"+newID, nil, &verification); err != nil {
		return err
	}
...
func call(client *http.Client, method, url string, body any, target any) error {
